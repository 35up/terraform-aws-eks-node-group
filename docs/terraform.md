...
| <a name="input_ami_release_version"></a> [ami\_release\_version](#input\_ami\_release\_version) | The EKS AMI "release version" to use. Defaults to the latest recommended version.<br/>For Amazon Linux, it is the "Release version" from [Amazon AMI Releases](https://github.com/awslabs/amazon-eks-ami/releases)<br/>For Bottlerocket, it is the release tag from [Bottlerocket Releases](https://github.com/bottlerocket-os/bottlerocket/releases) without the "v" prefix.<br/>Note that unlike AMI names, release versions never include the "v" prefix.<br/>Examples:<br/>  AL2: 1.29.3-20240531<br/>  Bottlerocket: 1.2.0 or 1.2.0-ccf1b754 | `list(string)` | `[]` | no |
| <a name="input_ami_type"></a> [ami\_type](#input\_ami\_type) | Type of Amazon Machine Image (AMI) associated with the EKS Node Group.<br/>Defaults to `AL2_x86_64`. Valid values: `AL2_x86_64, AL2_x86_64_GPU, AL2_ARM_64, CUSTOM, BOTTLEROCKET_ARM_64, BOTTLEROCKET_x86_64, BOTTLEROCKET_ARM_64_NVIDIA, BOTTLEROCKET_x86_64_NVIDIA, AL2023_x86_64_STANDARD, AL2023_ARM_64_STANDARD`. | `string` | `"AL2_x86_64"` | no |
| <a name="input_attach_canned_eks_cni_policy"></a> [attach\_canned\_eks\_cni\_policy](#input\_attach\_canned\_eks\_cni\_policy) | Toggle for attaching the AWS managed EKS CNI policy to the worker node's<br/>role. If using the EKS addons to handle the AWS VPC CNI plugin, AWS<br/>recommends that this policy should not be attached directly to the node's<br/>IAM role but rather be specified separately in the addon. | `bool` | `false` | no |
...
| <a name="input_ebs_optimized"></a> [ebs\_optimized](#input\_ebs\_optimized) | If toggled, the launched EC2 instance will be EBS-optimized | `bool` | `true` | no |
...
| <a name="input_node_role_explicit_self_trust"></a> [node\_role\_explicit\_self\_trust](#input\_node\_role\_explicit\_self\_trust) | Toggle for creating an explicit statement in the trust policy so that the Node Group IAM role can assume itself. | `bool` | `true` | no |
